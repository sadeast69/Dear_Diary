...
public final class InventoryDiaryButtonWidget extends AbstractWidget {
    ...
    public InventoryDiaryButtonWidget(
        ...
        super(x, y, SIZE, SIZE, Component.translatable("screen.dear_diary.inventory_button"));
    ...
    public boolean handleScreenMouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        return mouseDragged(mouseX, mouseY, button, deltaX, deltaY);
    }

    public boolean handleScreenMouseReleased(double mouseX, double mouseY, int button) {
    ...
    protected void renderWidget(GuiGraphics context, int mouseX, int mouseY, float delta) {
        ...
        if (hovered && !dragging) {
            context.renderTooltip(textRenderer, Component.translatable("screen.dear_diary.inventory_button.tooltip"), mouseX, mouseY);
        }
    }
    ...
    private void renderFreeButton(GuiGraphics context, boolean hovered) {
    ...
    private void renderAttachedButton(GuiGraphics context, boolean hovered, InventoryAnchorSide side) {
    ...
    private void drawAttachedConnector(GuiGraphics context, InventoryAnchorSide side, int fill) {
        switch (side) {
            case LEFT -> {
    ...
    private void drawAttachedFrame(GuiGraphics context, InventoryAnchorSide side, int fill) {
        context.fill(getX(), getY(), getRight(), getBottom(), BORDER_DARK);
        context.fill(getX() + 1, getY() + 1, getRight() - 1, getBottom() - 1, BORDER_SHADOW);
        context.fill(getX() + 2, getY() + 2, getRight() - 2, getBottom() - 2, fill);
        context.fill(getX() + 2, getY() + 2, getRight() - 3, getY() + 3, BORDER_LIGHT);
    ...
    private void softenAttachedEdge(GuiGraphics context, InventoryAnchorSide side, int fill) {
        switch (side) {
    ...
    private void renderIcon(GuiGraphics context, int pressOffset) {
        int iconX = getX() + (width - 16) / 2 + pressOffset;
        int iconY = getY() + (height - 16) / 2 + pressOffset;
        if (customIconAvailable()) {
            context.blit(ICON_TEXTURE, iconX, iconY, 0.0F, 0.0F, 16, 16, 16, 16);
            return;
        }

        context.renderItem(FALLBACK_ICON, iconX, iconY);
    }
    ...
    private void renderDebugOverlay(GuiGraphics context) {
        if (!debugOverlay) {
            return;
        }

        context.renderOutline(inventoryBounds.x(), inventoryBounds.y(), inventoryBounds.width(), inventoryBounds.height(), DEBUG_INVENTORY);
        ...
        context.drawString(textRenderer, mode, getX(), Math.max(0, getY() - 10), DEBUG_TEXT, true);
    }
