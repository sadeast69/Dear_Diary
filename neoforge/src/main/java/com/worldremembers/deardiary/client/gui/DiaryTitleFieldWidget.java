...
final class DiaryTitleFieldWidget extends AbstractWidget {
    ...
    DiaryTitleFieldWidget(Font textRenderer, int x, int y, int width, int height, Component message) {
    ...
    protected void renderWidget(GuiGraphics context, int mouseX, int mouseY, float delta) {
        int textX = getX();
        int textY = getY() + Math.max(0, (height - textRenderer.lineHeight) / 2);
        int innerWidth = Math.max(1, width);

        if (text.isEmpty() && !isFocused()) {
            context.drawString(textRenderer, textRenderer.plainSubstrByWidth(placeholder.getString(), innerWidth), textX, textY, PLACEHOLDER_COLOR, false);
            return;
        }

        context.drawString(textRenderer, visibleText(cursorDisplayText(), innerWidth), textX, textY, TEXT_COLOR, false);
    }
    ...
    private String visibleText(String value, int maxWidth) {
        ...
        return textRenderer.plainSubstrByWidth(value, maxWidth, true);
    }
